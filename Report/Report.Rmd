...
estacoes[which(estacoes$WSlocality == 'Sao Carlos'), 'Bioma'] <- 'Cerrado'
...
```
Criando uma segunda matrix para covariacao do tcmax
```{r cov.tcmax,cache=TRUE,echo=FALSE,warning=FALSE,message=FALSE}
ordem.Data <- aggregate(tcmax ~ ordem, data = dados, mean)
classe.Data<-aggregate(tcmax ~ classe, data = dados, mean)
